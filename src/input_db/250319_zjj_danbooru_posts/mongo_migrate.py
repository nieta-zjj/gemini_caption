...
class MongoDBMigration:
    ...
    async def migrate_batch(self, skip, limit):
        ...
        try:
            # 从源数据库读取一批数据
            cursor = self.source_collection.find({}).sort([("id", 1)]).skip(skip).limit(limit)
            documents = []
            ...
            # 执行批量写入
            result = await self.target_collection.bulk_write(bulk_operations, ordered=False)
    ...
    async def migrate(self):
        ...
        try:
            ...
            logger.info(f"将分 {batches} 批进行迁移")
            ...
            async def process_batch(batch_index):
                async with semaphore:
                    skip = batch_index * self.batch_size
                    batch_result = await self.migrate_batch(skip, self.batch_size)
                    pbar.update(min(self.batch_size, total_count - skip))
                    return batch_result

            # 创建所有批次的任务
            tasks = [process_batch(i) for i in range(batches)]
...
async def main_async():
    ...
    migration = MongoDBMigration(
        ...
        batch_size=2000,  # 每批处理100条记录
        concurrency=5    # 同时处理5个批次
    )
